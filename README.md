...
> ⚠️ **설치가 끝난 뒤에는 반드시 지금 열려 있는 PowerShell 창을 닫고, 새 PowerShell 창을 열어서 `claude`, `codex`, `gemini` 등을 실행해주세요.**
> 같은 창에서 바로 실행하면 `claude : 용어가 cmdlet, 함수... 이름으로 인식되지 않습니다` 라는 오류가 나는 것이 정상입니다 (버그 아님).
...
### 설치 후 `claude`(또는 codex, gemini)가 "인식되지 않는 명령어"라고 나와요

**정상적인 현상입니다.** 설치 스크립트를 실행한 그 PowerShell/터미널 창은 설치 전의 PATH 정보를 그대로 갖고 있어서, 새로 설치된 명령어를 바로 인식하지 못합니다.

✅ 해결: **현재 창을 닫고 새 PowerShell(또는 터미널)을 연 뒤** 다시 실행해주세요.
